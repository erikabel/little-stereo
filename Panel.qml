...
Panel {
  id: root
  moduleName: "io.github.erikabel.little-stereo"
  ipcTarget: "io.github.erikabel.little-stereo"
  ...
  function outputHasBalance(node) {
    if (!node) return false
    return Math.abs(Number(outputBalances[String(node.name || "")] || 0)) > 0
  }

  function outputBalanceDescription(node) {
    if (!node) return "Centered balance"
    var value = Number(outputBalances[String(node.name || "")] || 0)
    if (value < 0) return Math.abs(value) + "% left balance"
    if (value > 0) return value + "% right balance"
  ...
  function setOutputBalance(value) {
    outputBalance = Math.max(-1, Math.min(1, value))
    pendingOutputBalance = Math.round(outputBalance * 100)
    balanceApplyTimer.restart()
  }
      ...
      onStreamFinished: {
        ...
        if (lines !== "") {
          ...
          for (var i = 0; i < rows.length; i++) {
            ...
            if (fields.length >= 2) values[fields[0]] = Number(fields[1])
          }
        }
        root.outputBalances = values
      }
    }
  }
  ...
  Timer {
    id: balanceApplyTimer
    interval: 80
    repeat: false
    onTriggered: Quickshell.execDetached([
      root.helperPath,
      "balance",
      String(root.pendingOutputBalance)
    ])
  }
      ...
      ScrollView {
        ...
        Column {
          ...
          Column {
            ...
            Item {
              id: channelSwapSetting
              width: parent.width
              implicitHeight: Math.max(channelSwapLabels.implicitHeight, channelSwapSwitch.implicitHeight) + Style.space(8)

              Column {
                id: channelSwapLabels
                anchors.left: parent.left
                anchors.leftMargin: Style.space(6)
                anchors.right: channelSwapSwitch.left
            ...
            Column {
              ...
              Item {
                width: parent.width
                implicitHeight: Math.max(balanceTitle.implicitHeight, balanceValue.implicitHeight)
              ...
              Row {
                ...
                PanelSlider {
                  id: balanceSlider
                  bar: root.bar
                  width: parent.width - balanceLeftLabel.implicitWidth - balanceRightLabel.implicitWidth - parent.spacing * 2
  ...
  component SinkRow: CursorSurface {
    ...
    Row {
      ...
      Row {
        id: outputBadges
        z: 2
        visible: sinkRow.channelsSwapped || sinkRow.hasBalance
        spacing: Style.space(5)
        anchors.verticalCenter: parent.verticalCenter
        ...
        Item {
          ...
          implicitHeight: Math.max(balanceBadgeIcon.implicitHeight, Style.space(22))
          width: implicitWidth
        ...
        Item {
          visible: sinkRow.channelsSwapped
          implicitWidth: swapBadgeIcon.implicitWidth + Style.space(8)
          implicitHeight: Math.max(swapBadgeIcon.implicitHeight, Style.space(22))
          width: implicitWidth
          height: implicitHeight
